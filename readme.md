...
Vuex 是一个专门为 Vue.js 应用程序开发的状态管理库。它采用集中式存储管理应用的所有组件的状态，并以相应的规则保证状态以可预测的方式发生变化。

state：存储数据的状态对象，类似于组件中的 data 属性，但是 state 中的数据可以被任何组件访问，而不用通过父子组件传递。

mutations：包含多个更新 state 的同步方法的对象，用于改变 state 中的数据。只能包含同步方法，否则会导致状态不可预测。

actions：包含多个提交 mutation 的方法的对象，用于处理异步操作和复杂逻辑。可以包含异步方法。

getters：包含多个获取 state 的计算属性的方法的对象，用于从 store 中获取数据并进行计算。类似于组件中的 computed 属性。

## vue3 vue-router 钩子函数

Vue Router 是 Vue.js 官方的路由管理器，用于实现单页应用程序中的页面切换和跳转。

Vue Router 提供了多个钩子函数，可以在路由变化时执行相应的操作。下面是 Vue Router 中的几个常用的钩子函数：

beforeRouteEnter(to, from, next)：在路由进入前被调用，可以在此函数中获取组件实例和路由信息，但是无法访问 this。必须调用 next() 才能进入路由。

beforeRouteUpdate(to, from, next)：在路由更新前被调用，可以在此函数中处理路由参数的变化，如获取新的 id 等。也必须调用 next() 才能更新路由。

beforeRouteLeave(to, from, next)：在路由离开前被调用，可以在此函数中进行离开前的确认或清理操作。必须调用 next() 才能离开路由。

afterEach(to, from)：在路由完成后被调用，可以在此函数中进行一些后置操作，如数据的清理或恢复等。

以上钩子函数都可以通过定义在路由配置中的方式来使用。
